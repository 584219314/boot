 ...
 * 
 * @author danfeng.zhou
 * @version $Id: StringUtil.java, v 0.1 2017年4月27日 下午6:07:02 danfeng.zhou Exp $
 */
public class StringUtil {
	 ...
	 * @return
	 */
	public static Integer getAgeByIdNo(String idNo, String currentDate) {
		String birthday = getBirthdayByID(idNo);
		...
		try {
			...
			paltDate = dateFormat.parse(currentDate);
		} catch (ParseException e) {
			throw new RuntimeException("format date error!", e);
		}
		return getAgeByBirthday(date, paltDate);
	}
	...
	public static String getBirthdayByID(String idNo) {
		if (idNo.length() != 18) {
			throw new BizException("身份证输入有误");
		} else {
	...
	public static int getAgeByBirthday(Date birthday, Date currentDate) {
		...
		if (cal.before(birthday)) {
			throw new BizException(BizException.PARAM_ERROR.getCode(),
					"出生日期有误，出生日期大于当前系统日期");
		}
	...
	public static double parseStringToDouble(String number) {
		if (number == null) {
			...
			String temp = StringUtil.replace(number, ",", "").trim();
	 ...
	 * @return
	 */
	public static int calcMonthBet2(String date1, String date2) {
	 ...
	 * @return
	 */
	public static int calcMonthBet(String date1, String date2) {
	 ...
	 * @return
	 */
	public static char[] retrunbackchar(char stringArr[]) {
		char temp[] = new char[stringArr.length];
		for (int i = 0; i < stringArr.length; i++) {
			temp[i] = stringArr[stringArr.length - i - 1];
		}
		return temp;
	}
	...
	/**
	 * 生成请求流水号 取值：三位随机数+通过System.nanoTime()返回最准确的可用系统计时器的当前值，以毫微秒为单位（15位）
	 * 
	 * @return
	 */
	public static String createReqNbr() {
		return createRandomNum(3) + System.nanoTime();
	}
}
